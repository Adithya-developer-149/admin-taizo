...
const EmployerLeadPost = ({ reloeadpage, oncloseLead }) => {
  ...
  const checkfield = (e) => {
    ...
    // const isAdminID =
    //   leadData.fromAdminId.val.length < 0
    const isValidCompanyName = companynameInputRegex.test(
      leadData.companyName.val
    );
    const isValidpaymentDays = paymentDaysRegex.test(leadData.paymentDays.val);
    const isValidEmail = emailRegex.test(leadData.emailId.val);
    // const isValidccEmail = emailRegex.test(leadData.ccMail.val);
    const isValidcontactPersonName = textInputRegex.test(
      leadData.contactPersonName.val
    );

    let isvalidintro = true;
    if (isSuperAdmin) {
      isvalidintro = leadData.fromAdminId.val.length !== 0;
    }

    setLeadData((prev) => ({
      ...prev,
      mobileNumber: { val: prev.mobileNumber.val, err: !isValidMobile },
      companyName: { val: prev.companyName.val, err: !isValidCompanyName },
      emailId: { val: prev.emailId.val, err: !isValidEmail },
      // ccMail: { val: prev.ccMail.val, err: !isValidccEmail },
      paymentDays: { val: prev.paymentDays.val, err: !isValidpaymentDays },
      contactPersonName: {
        val: prev.contactPersonName.val,
        err: !isValidcontactPersonName,
      },
      fromAdminId: { val: prev.fromAdminId.val, err: !isvalidintro },
    }));
    ...
    if (
      ...
      console.log("Mobile number, email, company name, or intro is invalid.");
      return;
    }
              ...
              <div className="col-lg-12 col-md-12 col-sm-12 ">
                <Box sx={{ minWidth: 120 }}>
                  ...
                  >
                    ...
                    {leadData.fromAdminId && leadData.fromAdminId.err && (
                      <FormHelperText>
